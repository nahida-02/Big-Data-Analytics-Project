...
def state_path(
    ...
    return f"{state_root}/{state_id.lower()}_{state_name}"
...
def load_partition_states(
    ...
    for state_id, state_name, _, _, data_type in STATE_CONFIGS:
        path = state_path(state_root, state_id, state_name)

        if not DeltaTable.isDeltaTable(spark, path):
            raise RuntimeError(f"Missing Delta state: {path}")

        cast_type = "long" if data_type == "long" else "double"
...
def main() -> None:
    ...
    spark = (
        ...
        .appName("StateGuardExactIncrementalValidation")
        .getOrCreate()
    )
    ...
    state_root = args.initial_state_root.rstrip("/")
    ...
    print("=" * 78)
    print("STATEGUARD EXACT CDF-BASED INCREMENTAL VALIDATION")
    print("=" * 78)
    ...
    print(f"Initial state root: {state_root}")
    print(
    ...
    # ------------------------------------------------------------------
    # Phase A: Read and compact CDF into exact base/final row images.
    # This removes intermediate updates and yields the true net change
    # between the two table versions.
    # ------------------------------------------------------------------
    ...
    missing_cdf_columns = sorted(
        required_cdf_columns.difference(raw_cdf.columns)
    )
    ...
    if duplicate_net_roles != 0:
        raise RuntimeError(
            "CDF compaction produced duplicate base/final row images."
        )
    ...
    base_partition_state = (
        load_partition_states(spark, state_root)
        .persist(StorageLevel.MEMORY_AND_DISK)
    )
    ...
    key_state_path = (
        f"{state_root}/s12_exact_key_frequency"
    )
    ...
    (
        updated_partition_state.write.format("delta")
        .mode("overwrite")
    ...
    summary_df = spark.createDataFrame(
        [
            (
                ...
                (
                    float(speedup_vs_full)
                    if speedup_vs_full is not None
                    else None
                ),
                required_int(
                    initial_state_summary,
    ...
    print(
        "SPEEDUP_VS_FULL="
        f"{speedup_vs_full:.6f}"
        if speedup_vs_full is not None
        else "SPEEDUP_VS_FULL="
    )
    print(
        "UPDATED_PARTITION_STATE_SIZE_BYTES="
